...
use mcrs_minecraft_item::{Held, Holds, SlotTable, mutate};

use common::{holder, spawn, world};
...
#[test]
fn insert_move_and_swap_keep_the_table_exact() {
    let mut world = world();
    ...
    mutate::move_stack(&mut world, b, chest, 1).unwrap();
    assert_eq!(cells(&world, chest), [(0, a), (1, b)]);
    assert_eq!(world.get::<Holds>(chest).unwrap().entities(), [a, b]);

    mutate::move_stack(&mut world, a, chest, 3).unwrap();
    assert_eq!(cells(&world, chest), [(1, b), (3, a)]);

    mutate::detach(&mut world, a);
    mutate::move_stack(&mut world, b, chest, 3).unwrap();
    mutate::move_stack(&mut world, a, chest, 1).unwrap();
...
fn despawning_a_holder_cascades_through_nested_stacks() {
    ...
    world.entity_mut(shulker).insert(SlotTable::fixed(27));
    let pickaxe = spawn(&mut world, "diamond_pickaxe", 1);
    let bundle = spawn(&mut world, "bundle", 1);
    world.entity_mut(bundle).insert(SlotTable::list());
    let apple = spawn(&mut world, "apple", 3);
    mutate::move_stack(&mut world, shulker, chest, 0).unwrap();
    mutate::move_stack(&mut world, pickaxe, shulker, 1).unwrap();
    mutate::move_stack(&mut world, bundle, shulker, 2).unwrap();
    mutate::move_stack(&mut world, apple, bundle, 0).unwrap();
    world.despawn(chest);
...
fn a_list_grows_on_append_and_keeps_a_hole() {
    ...
    assert!(matches!(
        mutate::move_stack(&mut world, a, bundle, 1),
        Err(mutate::MoveError::OutOfRange { index: 1, .. })
    ));
    mutate::move_stack(&mut world, a, bundle, 0).unwrap();
    mutate::move_stack(&mut world, b, bundle, 1).unwrap();
    mutate::move_stack(&mut world, c, bundle, 2).unwrap();
    assert_eq!(world.get::<SlotTable>(bundle).unwrap().len(), 3);
    mutate::detach(&mut world, b);
    ...
    mutate::move_stack(&mut world, d, bundle, 3).unwrap();
...
fn a_replaced_table_is_rebuilt_from_held() {
    ...
    let b = spawn(&mut world, "dirt", 1);
    assert!(matches!(
        mutate::move_stack(&mut world, b, chest, 1),
        Err(mutate::MoveError::Occupied { by, .. }) if by == a
    ));
}
