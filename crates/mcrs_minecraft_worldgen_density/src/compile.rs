...
impl<'a> Compiler<'a> {
    ...
    pub fn positional_random(&self) -> RandomSource {
        self.random.clone()
    }
    ...
    pub fn hashed_random(&self, name: &str) -> RandomSource {
        let mut root = self.random.clone();
        root.fork_hash(name)
    }
...
pub(crate) mod tests {
    ...
    use mcrs_minecraft_worldgen_noise::strata::{AXIS_X, AXIS_Y, AXIS_Z};
