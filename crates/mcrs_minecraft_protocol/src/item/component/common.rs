...
/// A kind not implemented yet: every codec path fails loudly and the test
/// harness gets no samples for it.
    ...
    ($($ty:ident),* $(,)?) => {$(
        ...
        impl $ty {
            fn unimplemented() -> String {
                format!(
                    "{} is not implemented",
                    <$ty as $crate::item::kind::ItemDataComponent>::KIND.id()
                )
            }
        }
        ...
        impl serde::Serialize for $ty {
            fn serialize<S: serde::Serializer>(&self, _: S) -> Result<S::Ok, S::Error> {
                Err(serde::ser::Error::custom(Self::unimplemented()))
            }
        }

        impl<'de> serde::Deserialize<'de> for $ty {
            fn deserialize<D: serde::Deserializer<'de>>(_: D) -> Result<Self, D::Error> {
                Err(serde::de::Error::custom(Self::unimplemented()))
            }
        }
        ...
        impl $crate::item::ctx::EncodeCtx for $ty {
            fn encode_ctx(
                ...
                anyhow::bail!("{}", Self::unimplemented())
            }
        }
