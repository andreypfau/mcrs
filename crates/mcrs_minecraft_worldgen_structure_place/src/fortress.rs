...
fn bridge_crossing<W: WorldGenVolume>(b: &FortressBlocks, c: &mut PieceCanvas<'_, W>) {
    let nb = &b.bricks;
    c.solid(nb, [7, 3, 0], [11, 4, 18]);
    c.solid(nb, [0, 3, 7], [18, 4, 11]);
    c.solid(&b.air, [8, 5, 0], [10, 7, 18]);
    c.solid(&b.air, [0, 5, 8], [18, 7, 10]);
    c.solid(nb, [7, 5, 0], [7, 5, 7]);
    c.solid(nb, [7, 5, 11], [7, 5, 18]);
    c.solid(nb, [11, 5, 0], [11, 5, 7]);
    c.solid(nb, [11, 5, 11], [11, 5, 18]);
    c.solid(nb, [0, 5, 7], [7, 5, 7]);
    c.solid(nb, [11, 5, 7], [18, 5, 7]);
    c.solid(nb, [0, 5, 11], [7, 5, 11]);
    c.solid(nb, [11, 5, 11], [18, 5, 11]);
    c.solid(nb, [7, 2, 0], [11, 2, 5]);
    c.solid(nb, [7, 2, 13], [11, 2, 18]);
    c.solid(nb, [7, 0, 0], [11, 1, 3]);
    c.solid(nb, [7, 0, 15], [11, 1, 18]);
    for x in 7..=11 {
        for z in 0..=2 {
            fill_down(b, c, x, z);
    ...
    c.solid(nb, [0, 2, 7], [5, 2, 11]);
    c.solid(nb, [13, 2, 7], [18, 2, 11]);
    c.solid(nb, [0, 0, 7], [3, 1, 11]);
    c.solid(nb, [15, 0, 7], [18, 1, 11]);
    for x in 0..=2 {
        for z in 7..=11 {
            fill_down(b, c, x, z);
...
fn bridge_end_filler<W: WorldGenVolume>(b: &FortressBlocks, c: &mut PieceCanvas<'_, W>, seed: i32) {
    ...
    for x in 0..=4 {
        for y in 3..=4 {
            ...
            c.solid(nb, [x, y, 0], [x, y, z]);
        }
    }
    let z = own.next_i32_bound(8);
    c.solid(nb, [0, 5, 0], [0, 5, z]);
    let z = own.next_i32_bound(8);
    c.solid(nb, [4, 5, 0], [4, 5, z]);
    for x in 0..=4 {
        let z = own.next_i32_bound(5);
        c.solid(nb, [x, 2, 0], [x, 2, z]);
    }
    for x in 0..=4 {
        for y in 0..=1 {
...
fn bridge_straight<W: WorldGenVolume>(b: &FortressBlocks, c: &mut PieceCanvas<'_, W>) {
    let nb = &b.bricks;
    c.solid(nb, [0, 3, 0], [4, 4, 18]);
    c.solid(&b.air, [1, 5, 0], [3, 7, 18]);
    c.solid(nb, [0, 5, 0], [0, 5, 18]);
    c.solid(nb, [4, 5, 0], [4, 5, 18]);
    c.solid(nb, [0, 2, 0], [4, 2, 5]);
    c.solid(nb, [0, 2, 13], [4, 2, 18]);
    c.solid(nb, [0, 0, 0], [4, 1, 3]);
    c.solid(nb, [0, 0, 15], [4, 1, 18]);
    for x in 0..=4 {
        for z in 0..=2 {
            fill_down(b, c, x, z);
            fill_down(b, c, x, 18 - z);
    ...
    c.solid(&b.fence_nse, [0, 1, 1], [0, 4, 1]);
    c.solid(&b.fence_nse, [0, 3, 4], [0, 4, 4]);
    c.solid(&b.fence_nse, [0, 3, 14], [0, 4, 14]);
    c.solid(&b.fence_nse, [0, 1, 17], [0, 4, 17]);
    c.solid(&b.fence_nsw, [4, 1, 1], [4, 4, 1]);
    c.solid(&b.fence_nsw, [4, 3, 4], [4, 4, 4]);
    c.solid(&b.fence_nsw, [4, 3, 14], [4, 4, 14]);
    c.solid(&b.fence_nsw, [4, 1, 17], [4, 4, 17]);
}
...
fn corridor_stairs<W: WorldGenVolume>(b: &FortressBlocks, c: &mut PieceCanvas<'_, W>) {
    ...
    for step in 0..=9 {
        ...
        c.solid(nb, [0, 0, step], [4, floor, step]);
        c.solid(&b.air, [1, floor + 1, step], [3, roof - 1, step]);
        if step <= 6 {
            c.place(&b.stairs_south, 1, floor + 1, step);
            c.place(&b.stairs_south, 2, floor + 1, step);
            c.place(&b.stairs_south, 3, floor + 1, step);
        }
        c.solid(nb, [0, roof, step], [4, roof, step]);
        c.solid(nb, [0, floor + 1, step], [0, roof - 1, step]);
        c.solid(nb, [4, floor + 1, step], [4, roof - 1, step]);
        if step & 1 == 0 {
            c.solid(&b.fence_ns, [0, floor + 2, step], [0, floor + 3, step]);
            c.solid(&b.fence_ns, [4, floor + 2, step], [4, floor + 3, step]);
        }
        for x in 0..=4 {
...
fn corridor_balcony<W: WorldGenVolume>(b: &FortressBlocks, c: &mut PieceCanvas<'_, W>) {
    let nb = &b.bricks;
    c.solid(nb, [0, 0, 0], [8, 1, 8]);
    c.solid(&b.air, [0, 2, 0], [8, 5, 8]);
    c.solid(nb, [0, 6, 0], [8, 6, 5]);
    c.solid(nb, [0, 2, 0], [2, 5, 0]);
    c.solid(nb, [6, 2, 0], [8, 5, 0]);
    c.solid(&b.fence_we, [1, 3, 0], [1, 4, 0]);
    c.solid(&b.fence_we, [7, 3, 0], [7, 4, 0]);
    c.solid(nb, [0, 2, 4], [8, 2, 8]);
    c.solid(&b.air, [1, 1, 4], [2, 2, 4]);
    c.solid(&b.air, [6, 1, 4], [7, 2, 4]);
    c.solid(&b.fence_we, [1, 3, 8], [7, 3, 8]);
    c.place(&b.fence_se, 0, 3, 8);
    c.place(&b.fence_sw, 8, 3, 8);
    c.solid(&b.fence_ns, [0, 3, 6], [0, 3, 7]);
    c.solid(&b.fence_ns, [8, 3, 6], [8, 3, 7]);
    c.solid(nb, [0, 3, 4], [0, 5, 5]);
    c.solid(nb, [8, 3, 4], [8, 5, 5]);
    c.solid(nb, [1, 3, 5], [2, 5, 5]);
    c.solid(nb, [6, 3, 5], [7, 5, 5]);
    c.solid(&b.fence_we, [1, 4, 5], [1, 5, 5]);
    c.solid(&b.fence_we, [7, 4, 5], [7, 5, 5]);
    for z in 0..=5 {
        for x in 0..=8 {
            fill_down(b, c, x, z);
        }
...
fn castle_hall<W: WorldGenVolume>(b: &FortressBlocks, c: &mut PieceCanvas<'_, W>) {
    let nb = &b.bricks;
    c.solid(nb, [0, 3, 0], [12, 4, 12]);
    c.solid(&b.air, [0, 5, 0], [12, 13, 12]);
    c.solid(nb, [0, 5, 0], [1, 12, 12]);
    c.solid(nb, [11, 5, 0], [12, 12, 12]);
    c.solid(nb, [2, 5, 11], [4, 12, 12]);
    c.solid(nb, [8, 5, 11], [10, 12, 12]);
    c.solid(nb, [5, 9, 11], [7, 12, 12]);
    c.solid(nb, [2, 5, 0], [4, 12, 1]);
    c.solid(nb, [8, 5, 0], [10, 12, 1]);
    c.solid(nb, [5, 9, 0], [7, 12, 1]);
    c.solid(nb, [2, 11, 2], [10, 12, 10]);
}

fn castle_rail<W: WorldGenVolume>(b: &FortressBlocks, c: &mut PieceCanvas<'_, W>) {
    let nb = &b.bricks;
    for i in (1..=11).step_by(2) {
        c.solid(&b.fence_we, [i, 10, 0], [i, 11, 0]);
        c.solid(&b.fence_we, [i, 10, 12], [i, 11, 12]);
        c.solid(&b.fence_ns, [0, 10, i], [0, 11, i]);
        c.solid(&b.fence_ns, [12, 10, i], [12, 11, i]);
        c.place(nb, i, 13, 0);
        c.place(nb, i, 13, 12);
        c.place(nb, 0, 13, i);
        c.place(nb, 12, 13, i);
    ...
    for z in (3..=9).step_by(2) {
        c.solid(&b.fence_nsw, [1, 7, z], [1, 8, z]);
        c.solid(&b.fence_nse, [11, 7, z], [11, 8, z]);
    }
}

fn castle_foundation<W: WorldGenVolume>(b: &FortressBlocks, c: &mut PieceCanvas<'_, W>) {
    let nb = &b.bricks;
    c.solid(nb, [4, 2, 0], [8, 2, 12]);
    c.solid(nb, [0, 2, 4], [12, 2, 8]);
    c.solid(nb, [4, 0, 0], [8, 1, 3]);
    c.solid(nb, [4, 0, 9], [8, 1, 12]);
    c.solid(nb, [0, 0, 4], [3, 1, 8]);
    c.solid(nb, [9, 0, 4], [12, 1, 8]);
    for x in 4..=8 {
        for z in 0..=2 {
            fill_down(b, c, x, z);
...
fn castle_entrance<W: WorldGenVolume>(b: &FortressBlocks, c: &mut PieceCanvas<'_, W>) {
    let nb = &b.bricks;
    castle_hall(b, c);
    c.solid(&b.fence, [5, 8, 0], [7, 8, 0]);
    castle_rail(b, c);
    castle_foundation(b, c);
    c.solid(nb, [5, 5, 5], [7, 5, 7]);
    c.solid(&b.air, [6, 1, 6], [6, 4, 6]);
    c.place(nb, 6, 0, 6);
...
fn small_corridor_crossing<W: WorldGenVolume>(b: &FortressBlocks, c: &mut PieceCanvas<'_, W>) {
    let nb = &b.bricks;
    c.solid(nb, [0, 0, 0], [4, 1, 4]);
    c.solid(&b.air, [0, 2, 0], [4, 5, 4]);
    c.solid(nb, [0, 2, 0], [0, 5, 0]);
    c.solid(nb, [4, 2, 0], [4, 5, 0]);
    c.solid(nb, [0, 2, 4], [0, 5, 4]);
    c.solid(nb, [4, 2, 4], [4, 5, 4]);
    small_corridor_roof(b, c);
}

fn small_corridor_roof<W: WorldGenVolume>(b: &FortressBlocks, c: &mut PieceCanvas<'_, W>) {
    c.solid(&b.bricks, [0, 6, 0], [4, 6, 4]);
    for x in 0..=4 {
        for z in 0..=4 {
            fill_down(b, c, x, z);
        }
...
fn small_corridor_left_turn<W: WorldGenVolume, R: Random>(
    ...
    c.solid(nb, [0, 0, 0], [4, 1, 4]);
    c.solid(&b.air, [0, 2, 0], [4, 5, 4]);
    c.solid(nb, [4, 2, 0], [4, 5, 4]);
    c.solid(&b.fence_ns, [4, 3, 1], [4, 4, 1]);
    c.solid(&b.fence_ns, [4, 3, 3], [4, 4, 3]);
    c.solid(nb, [0, 2, 0], [0, 5, 0]);
    c.solid(nb, [0, 2, 4], [3, 5, 4]);
...
fn small_corridor<W: WorldGenVolume>(b: &FortressBlocks, c: &mut PieceCanvas<'_, W>) {
    ...
    c.solid(nb, [0, 0, 0], [4, 1, 4]);
    c.solid(&b.air, [0, 2, 0], [4, 5, 4]);
    c.solid(nb, [0, 2, 0], [0, 5, 4]);
    c.solid(nb, [4, 2, 0], [4, 5, 4]);
    c.solid(&b.fence_ns, [0, 3, 1], [0, 4, 1]);
    c.solid(&b.fence_ns, [0, 3, 3], [0, 4, 3]);
    c.solid(&b.fence_ns, [4, 3, 1], [4, 4, 1]);
    c.solid(&b.fence_ns, [4, 3, 3], [4, 4, 3]);
    small_corridor_roof(b, c);
}
...
fn stalk_room<W: WorldGenVolume>(b: &FortressBlocks, c: &mut PieceCanvas<'_, W>) {
    ...
    for i in 0..=6 {
        ...
        if (5..=8).contains(&z) {
            c.solid(nb, [5, 5, z], [7, i + 4, z]);
        } else if (9..=10).contains(&z) {
            c.solid(nb, [5, 8, z], [7, i + 4, z]);
        }
        if i >= 1 {
            c.solid(&b.air, [5, 6 + i, z], [7, 9 + i, z]);
        }
    }
    for x in 5..=7 {
        c.place(&b.stairs_north, x, 12, 11);
    }
    c.solid(&b.fence_nse, [5, 6, 7], [5, 7, 7]);
    c.solid(&b.fence_nsw, [7, 6, 7], [7, 7, 7]);
    c.solid(&b.air, [5, 13, 12], [7, 13, 12]);
    c.solid(nb, [2, 5, 2], [3, 5, 3]);
    c.solid(nb, [2, 5, 9], [3, 5, 10]);
    c.solid(nb, [2, 5, 4], [2, 5, 8]);
    c.solid(nb, [9, 5, 2], [10, 5, 3]);
    c.solid(nb, [9, 5, 9], [10, 5, 10]);
    c.solid(nb, [10, 5, 4], [10, 5, 8]);
    ...
    c.solid(&b.soul_sand, [3, 4, 4], [4, 4, 8]);
    c.solid(&b.soul_sand, [8, 4, 4], [9, 4, 8]);
    c.solid(&b.nether_wart, [3, 5, 4], [4, 5, 8]);
    c.solid(&b.nether_wart, [8, 5, 4], [9, 5, 8]);
    castle_foundation(b, c);
}

fn monster_throne<W: WorldGenVolume>(b: &FortressBlocks, c: &mut PieceCanvas<'_, W>) {
    let nb = &b.bricks;
    c.solid(&b.air, [0, 2, 0], [6, 7, 7]);
    c.solid(nb, [1, 0, 0], [5, 1, 7]);
    c.solid(nb, [1, 2, 1], [5, 2, 7]);
    c.solid(nb, [1, 3, 2], [5, 3, 7]);
    c.solid(nb, [1, 4, 3], [5, 4, 7]);
    c.solid(nb, [1, 2, 0], [1, 4, 2]);
    c.solid(nb, [5, 2, 0], [5, 4, 2]);
    c.solid(nb, [1, 5, 2], [1, 5, 3]);
    c.solid(nb, [5, 5, 2], [5, 5, 3]);
    c.solid(nb, [0, 5, 3], [0, 5, 8]);
    c.solid(nb, [6, 5, 3], [6, 5, 8]);
    c.solid(nb, [1, 5, 8], [5, 5, 8]);
    c.place(&b.fence_w, 1, 6, 3);
    c.place(&b.fence_e, 5, 6, 3);
    c.place(&b.fence_ne, 0, 6, 3);
    c.place(&b.fence_nw, 6, 6, 3);
    c.solid(&b.fence_ns, [0, 6, 4], [0, 6, 7]);
    c.solid(&b.fence_ns, [6, 6, 4], [6, 6, 7]);
    c.place(&b.fence_se, 0, 6, 8);
    c.place(&b.fence_sw, 6, 6, 8);
    c.solid(&b.fence_we, [1, 6, 8], [5, 6, 8]);
    c.place(&b.fence_e, 1, 7, 8);
    c.solid(&b.fence_we, [2, 7, 8], [4, 7, 8]);
...
fn room_crossing<W: WorldGenVolume>(b: &FortressBlocks, c: &mut PieceCanvas<'_, W>) {
    let nb = &b.bricks;
    c.solid(nb, [0, 0, 0], [6, 1, 6]);
    c.solid(&b.air, [0, 2, 0], [6, 7, 6]);
    c.solid(nb, [0, 2, 0], [1, 6, 0]);
    c.solid(nb, [0, 2, 6], [1, 6, 6]);
    c.solid(nb, [5, 2, 0], [6, 6, 0]);
    c.solid(nb, [5, 2, 6], [6, 6, 6]);
    c.solid(nb, [0, 2, 0], [0, 6, 1]);
    c.solid(nb, [0, 2, 5], [0, 6, 6]);
    c.solid(nb, [6, 2, 0], [6, 6, 1]);
    c.solid(nb, [6, 2, 5], [6, 6, 6]);
    c.solid(nb, [2, 6, 0], [4, 6, 0]);
    c.solid(&b.fence_we, [2, 5, 0], [4, 5, 0]);
    c.solid(nb, [2, 6, 6], [4, 6, 6]);
    c.solid(&b.fence_we, [2, 5, 6], [4, 5, 6]);
    c.solid(nb, [0, 6, 2], [0, 6, 4]);
    c.solid(&b.fence_ns, [0, 5, 2], [0, 5, 4]);
    c.solid(nb, [6, 6, 2], [6, 6, 4]);
    c.solid(&b.fence_ns, [6, 5, 2], [6, 5, 4]);
    for x in 0..=6 {
        for z in 0..=6 {
            fill_down(b, c, x, z);
        }
...
fn stairs_room<W: WorldGenVolume>(b: &FortressBlocks, c: &mut PieceCanvas<'_, W>) {
    let nb = &b.bricks;
    c.solid(nb, [0, 0, 0], [6, 1, 6]);
    c.solid(&b.air, [0, 2, 0], [6, 10, 6]);
    c.solid(nb, [0, 2, 0], [1, 8, 0]);
    c.solid(nb, [5, 2, 0], [6, 8, 0]);
    c.solid(nb, [0, 2, 1], [0, 8, 6]);
    c.solid(nb, [6, 2, 1], [6, 8, 6]);
    c.solid(nb, [1, 2, 6], [5, 8, 6]);
    c.solid(&b.fence_ns, [0, 3, 2], [0, 5, 4]);
    c.solid(&b.fence_ns, [6, 3, 2], [6, 5, 2]);
    c.solid(&b.fence_ns, [6, 3, 4], [6, 5, 4]);
    c.place(nb, 5, 2, 5);
    c.solid(nb, [4, 2, 5], [4, 3, 5]);
    c.solid(nb, [3, 2, 5], [3, 4, 5]);
    c.solid(nb, [2, 2, 5], [2, 5, 5]);
    c.solid(nb, [1, 2, 5], [1, 6, 5]);
    c.solid(nb, [1, 7, 1], [5, 7, 4]);
    c.solid(&b.air, [6, 8, 2], [6, 8, 4]);
    c.solid(nb, [2, 6, 0], [4, 8, 0]);
    c.solid(&b.fence_we, [2, 5, 0], [4, 5, 0]);
    for x in 0..=6 {
        for z in 0..=6 {
            fill_down(b, c, x, z);
