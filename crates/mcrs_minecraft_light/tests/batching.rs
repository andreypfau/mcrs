...
fn the_most_urgent_column_is_handed_out_first() {
    ...
    assert_eq!(
        queue.drain_batch(1, &[])[0].core.min,
...
fn a_column_keeps_the_most_urgent_priority_it_was_given() {
    ...
    assert_eq!(queue.len(), 2, "both pieces of work are still there");
}
...
fn a_column_that_would_blow_up_the_field_waits_for_the_next_batch() {
    ...
    // reach the far one.
    let batch = queue.drain_batch(6 * 3 * 3 * 4096, &[]);
    assert_eq!(batch.len(), 2, "the two neighbours went together");
    assert_eq!(queue.columns_waiting(), 1, "the far column waits");
    assert_eq!(
        queue.drain_batch(1, &[])[0].core.min,
...
fn a_column_under_work_in_flight_is_left_for_the_next_tick() {
    ...
    let in_flight = Influence::around(BlockPos::new(8, 8, 8))
        ...
        .expand(1)
        .section_aligned();
    let batch = queue.drain_batch(u64::MAX, &[in_flight]);
    ...
    assert_eq!(queue.columns_waiting(), 1);
}
...
fn disjoint_jobs_publish_in_any_order() {
    ...
    let older = concurrent
        .world
        .prepare_batch(queue.drain_batch(1, &[]))
    ...
    let batch = queue.drain_batch(1, &[older.area()]);
    assert!(
        !batch.is_empty(),
...
fn batches_filled_in_one_pass_do_not_overlap() {
    ...
    assert_eq!(queue.columns_waiting(), 1, "the fourth column waits");
