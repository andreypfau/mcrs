...
// for missing chunks). seed_initial_light read the stale heightmap and
...
// early if any chunk slot is still None, so the prime never runs on a partial
...
//           columns → inserts ChunkNeedsInitialLight on both chunks.
//           seed_initial_light fires for both chunks with the correct heightmap.
//           Cave chunk is Case C (all below surface) → sky_light=0.
...
/// and ChunkNeedsInitialLight insertion until the surface chunk arrives.
/// seed_initial_light then runs once with the fully-primed heightmap and
/// classifies the cave chunk as Case C (all below surface) → sky_light=0.
#[test]
fn cave_air_chunk_dark_when_surface_chunk_arrives_later() {
    ...
    // full data → inserts ChunkNeedsInitialLight on both chunks.
    // seed_initial_light fires for both with correct heightmap (surface=32).
    // Cave chunk: chunk_base_y=0, s=32 > chunk_top_y=15 → Case C → dark.
