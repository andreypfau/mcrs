...
const JSON_OUT_PATH: &str = "crates/mcrs_voxel_light/benches/results/memory-profile.json";
...
fn walk_ecs(app: &mut bevy_app::App) -> MemorySnapshot {
    ...
    for idx in world.query::<&ColumnIndex>().iter(world) {
        use mcrs_voxel_world::world::storage::column::{ColumnPos, ColumnSlot};
    ...
    let telemetry_iterations = mcrs_voxel_light::metrics::snapshot().iterations;
