...
mod tests {
    ...
    use mcrs_minecraft_worldgen_feature::placer::{BoxRegion, WorldStates, mask_of};
    ...
    fn volume() -> BoxRegion {
        let mut volume = BoxRegion::new(BlockPos::new(0, -64, 0), BlockPos::new(31, -50, 31), AIR);
    ...
    fn it_fills_one_chunk_square_at_one_height_and_only_over_air() {
        ...
        volume.set(BlockPos::new(3, -60, 4), STONE);

        let cfg = CompiledFillLayer {
            height: 4,
            state: SNOW,
        };
        assert!(place_fill_layer(&cfg, &mut volume, BlockPos::new(0, 0, 0)));

        assert_eq!(volume.get(BlockPos::new(0, -60, 0)), SNOW);
        assert_eq!(volume.get(BlockPos::new(15, -60, 15)), SNOW);
        assert_eq!(
            volume.get(BlockPos::new(3, -60, 4)),
            STONE,
            "a filled cell is left alone"
        );
        assert_eq!(
            volume.get(BlockPos::new(16, -60, 0)),
            AIR,
            "the square stops at sixteen"
        );
        assert_eq!(
            volume.get(BlockPos::new(0, -59, 0)),
            AIR,
    ...
    fn the_height_counts_from_the_window_floor() {
        ...
        place_fill_layer(&cfg, &mut volume, BlockPos::new(0, 0, 0));
        assert_eq!(volume.get(BlockPos::new(0, -64, 0)), SNOW);
    }
}
